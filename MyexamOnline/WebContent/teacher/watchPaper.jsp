...
<form action="../ManagementQuestion?method=findsaAnswer" method="post">
...
<%if (re == null || re.size() == 0) {
 ...
 <h4>四、简答题(20分)</h4>
	<%
List<ShortAnswerQuestions> record1 = (List<ShortAnswerQuestions>)session.getAttribute("saQPaper");
%>
<%if (record1 == null || record1.size() == 0) {
		out.println("<center><strong>还没有试题哦~</strong></center>");						
		}
	else {%>
		<% for (ShortAnswerQuestions r : record1) {	
	%>
   <ul class="list-group">
    <li class="list-group-item"><%=i %>、<%=r.getS_question() %></li>
    <li class="list-group-item">
    请填写:
