...
<span style="color:red;font-size: 20px">*如果删除或修改学生，与该学生相关的信息（如该班的学生个人信息、成绩和错题等）也会做出相应的删除或者修改</span>
<%
...
<%if (record == null || record.size() == 0) {
	...
	%>
     ...
     <td><%=r.getClassid() %></td>
     <td><%if(r.getPwd()==null) out.print("-"); else out.print(r.getPwd()); %>
...
<form method="post" action="../ManagementStudent?method=updateStudent">
 ...
 班级编号：<input type="text" name="classid" id="classid"><br>
...
function values(sid) {  
    ...
    var classid = document.getElementById("table").rows[id].cells[3].innerText;  
    var pwd = document.getElementById("table").rows[id].cells[4].innerText;  
    var phone = document.getElementById("table").rows[id].cells[5].innerText;  
    var address = document.getElementById("table").rows[id].cells[6].innerText;  
    var birthdate = document.getElementById("table").rows[id].cells[7].innerText;  
    var email = document.getElementById("table").rows[id].cells[8].innerText;  
    //alert(j_answer);
    //向模态框中传值  
