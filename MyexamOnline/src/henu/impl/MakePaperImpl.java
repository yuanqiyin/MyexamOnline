...
public class MakePaperImpl implements MakePaperIF {
	...
	public boolean makePaper(Paper p) {
		...
		String sql = "INSERT INTO paper (starttime,endtime,subjectid,papername,"
				...
				+ ",f1,f2,f3,f4,f5,j1,j2,j3,j4,j5,s1,s2,tid,classid) "
				+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		try {
			...
			ps.setInt(25, p.getS1());
			ps.setInt(26, p.getS2());
			ps.setString(27, p.getTid());
			ps.setInt(28, p.getClassid());
			result = ps.executeUpdate();
			ps.close();
		} catch (SQLException e) {
			e.printStackTrace();
		}
		//System.out.println(p.getPapername());
		DbcpPool.close();
		if(result>0)
		return true;
		else 
	...
	private Paper makesaQusetion(Paper p) {
		String sql = null;
		sql ="SELECT s_id FROM shortanswerquestions "
				+ "WHERE s_subjectid="+p.getSubjectid()+" and s_id >= (SELECT floor(RAND() * "
				+ "(SELECT MAX(s_id) FROM shortanswerquestions)))  "
				+ "ORDER BY s_id LIMIT 2";
		ResultSet rs = null;
        //String sb = "";
		try{
			rs = DbcpPool.executeQuery(sql);
			rs.next();
			p.setS1(rs.getInt("s_id"));
			rs.next();
			p.setS2(rs.getInt("s_id"));
		}catch(Exception e)
		{
			e.printStackTrace();
		}
