...
public class StatisticsImpl implements StatisticsIF{
	...
	{
		String sql = null;
		sql = "select avg(score),avg(scorses),min(score),min(scorses),max(score),max(scorses) from grade where paperid="+st.getPaperid()+"";
		ResultSet rs = null;
		try{
			rs = DbcpPool.executeQuery(sql);
			while(rs.next()){
			st.setAverscore((rs.getInt("avg(score)")+rs.getInt("avg(scorses)")));
			st.setHighestscore((rs.getInt("max(score)")+rs.getInt("max(scorses)")));
			st.setLowestscore((rs.getInt("min(score)")+rs.getInt("min(scorses)")));
		}
			rs.close();
		}catch(Exception e)
		{
