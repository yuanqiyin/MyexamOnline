...
<div class="container">
  ...
  <div class="container-fluid">
    ...
    <ul class="nav navbar-nav navbar-right">
		...
		<h3>简答题</h3><span style="color:red;">*如果要删除或修改题目，可能会出现一些题目被用做试卷而删除或修改失败的情况</span><br>
		<form action="../ManagementQuestion?method=findsaQuestion" method="post">
	...
	<div  class="jumbotron">	
<%
List<ShortAnswerQuestions> record = (List<ShortAnswerQuestions>)session.getAttribute("saList");
%>
<%if (record == null || record.size() == 0) {
    ...
    <tr class="warning">
     ...
     <td><%=r.getS_answer() %></td>   
     <td><%=r.getS_subjectname() %></td>    
     <td>
         <a href="#" data-toggle="modal" data-target="#myModal" onclick="values(<%=r.getS_id()%>)">修改</a>
         <a href="../ManagementQuestion?method=deletesaQuestion&s_id=<%=r.getS_id() %>">删除</a>
     </td>
    </tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">简答题</h4>
            </div>
            <form method="post" action="../ManagementQuestion?method=modifysaQuestion">
            <div class="modal-body">
            <input type="hidden" name="s_id" id="id" /> 
          题目：<input type="text" name="saQuestion" id="saQuestion" /><br>
	 答案：<input type="text" name="saAnswer" id="saAnswer" />
            </div>
...
function values(sid) {  
    ...
    document.getElementById("saQuestion").value = s_question;
    document.getElementById("saAnswer").value = s_answer;
