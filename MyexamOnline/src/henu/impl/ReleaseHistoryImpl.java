...
public class ReleaseHistoryImpl implements ReleaseHistoryIF{
	...
	public List<ShortAnswerQuestions> getsaQPaper(int paperid) {
		...
		String sql = null;
		sql = "select * from paper,shortanswerquestions where (s_id=s1 or s_id=s2 )"
				+ " and paperid="+paperid+" order by s_id";
		List<ShortAnswerQuestions> list = new ArrayList<ShortAnswerQuestions>();
		ResultSet rs = null;
		try{
			rs = DbcpPool.executeQuery(sql);
			while(rs.next()){
				ShortAnswerQuestions fq = new ShortAnswerQuestions();
			fq.setS_question(rs.getString("s_question"));
			list.add(fq);
			}
			rs.close();
		}catch(Exception e)
